...
def _get_serve_tf_examples_fn(model, tf_transform_output):
    ...
    def serve_tf_examples_fn(serialized_tf_examples):
        ...
        sanitized_input_names = [
            f.replace(
                ' ',
                '_').lower() +
            '_xf' for f in feature_spec.keys()]
        ...
        # Rename input tensor
        serving_default_signature._input_signature[0].name = 'examples'

        # Assign sanitized names to input tensors
        for i, input_tensor in enumerate(serving_default_signature.inputs):
            # Assign sanitized name to input tensor
            input_tensor._name = sanitized_input_names[i]
...
def run_fn(fn_args: FnArgs) -> None:
    ...
    es = tf.keras.callbacks.EarlyStopping(
        monitor='val_loss', mode='max', verbose=1, patience=10)
